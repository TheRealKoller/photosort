...
- **Nummernvergabe:** Die Nummer eines neuen Dokuments in `specs/decisions/`/`specs/architecture/` und die Kennung einer neuen Alembic-Revision kommen von `scripts/nummern.py`, nie aus einem eigenen Blick auf das Verzeichnis — parallel laufende Arbeitsstände sind sonst unsichtbar, und beide Seiten wählen dieselbe Nummer (ADR [`decisions/0108-nummer-folgt-dem-rang-des-arbeitsstands.md`](./specs/decisions/0108-nummer-folgt-dem-rang-des-arbeitsstands.md)). Exit `10` heißt: ein paralleler Stand führt bereits eine Nummer ab derselben Basis — die ausgegebene Nummer berücksichtigt ihn bereits, und die Datei wird unmittelbar angelegt, weil sie erst dadurch für die Nachbarn sichtbar wird. Die Nummer einer Feature-Spec kommt dagegen vom GitHub-Issue (ADR 0043) und fällt nicht unter diese Regel; der Zuteiler lehnt sie mit einer Meldung ab, statt still eine zu liefern.
