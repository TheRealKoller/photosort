 ...
 * Schriftstufen sind **Verbundtokens** vom Typ `typography`: Penpot
 ...
 * Wertekopie, die dieser Erzeuger verbietet; das Feld ist nur nicht mehr leer da, sondern gar
 * nicht.
 *
...
/**
 * EINZIGE BEWUSSTE UEBERSETZUNG DIESES ERZEUGERS: uebernommen wird die
...
export function buildTokens(css: string): TokenBuildResult {
  ...
  for (const [name, value] of parseDeclarations(extractBlock(css, '@theme {'))) {
    ...
    /*
     * `--spacing-header` ist KEIN Design-Token, sondern eine Layout-Konstante der App-Huelle: die Hoehe der fixierten Kopfzeile, aus der
