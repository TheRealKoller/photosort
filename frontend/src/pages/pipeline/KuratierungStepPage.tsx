...
/**
 * 1:1-Migration der bisherigen Kuratierungs-Section aus ProjectDetailPage.tsx
...
export function KuratierungStepPage() {
  ...
  const [topNPerEvent, setTopNPerEvent] = useState<number | ''>(DEFAULT_TOP_N)
  const effectiveTopNPerEvent = topNPerEvent === '' ? DEFAULT_TOP_N : topNPerEvent
    ...
    <div className="flex flex-col gap-8">
      <section className="flex flex-col items-start gap-3">
        ...
        <p className="text-sm text-text">
          Zeigt pro Foto-Moment die besten N Fotos — verworfene Fotos bleiben an ihrer Stelle
          sichtbar, und weitere Kandidaten lassen sich bei Bedarf einblenden.
        </p>

        <div className="flex flex-wrap items-end gap-3">
          <label htmlFor="top-n-per-event" className="flex flex-col gap-1 text-sm text-text">
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                if (!Number.isNaN(value)) {
                  setTopNPerEvent(Math.min(MAX_TOP_N, Math.max(MIN_TOP_N, Math.round(value))))
                }
              }}
              className="w-24"
            />
          ...
          <Button asChild variant="secondary">
            <Link to={`/projects/${project.id}/curate?topN=${effectiveTopNPerEvent}`}>
              Kuratierung öffnen
            </Link>
          </Button>
        </div>
