...
| 5c | Abgleich mit `main` (erster Zeitpunkt) | Findings behoben, vor dem Push | Skill `ship-feature` (Schritt 6.2) ruft `scripts/merge-main-into-branch.sh` | Hauptsession (rein lokales `git`) | — | immer; bei Exit `10`/`20` wiederholt der `developer`-Subagent seinen Qualitätscheck (Folgeauftrag „Abgleich mit `main`") |
...
| 7b | Abgleich mit `main` (zweiter, tragender Zeitpunkt) + Spec finalisieren (im selben PR) | Review + Copilot ausgewertet, Findings behoben | Skill `ship-feature` (Schritt 8; der Abgleich als erste Handlung, dann die `**Status:**`-Zeile der Spec-Datei) | Hauptsession (GitHub-Zugriff) | — | immer; Merge-Commit, etwaiger Konflikt-Fix und Finalisierung gebündelt im letzten Push, damit weder ein Nachzieh-PR noch ein zusätzlicher CI-Lauf entsteht |
| 7c | Auf das CI-Ergebnis warten, bei Rot begrenzt nachbessern | der letzte Push des Laufs | Skill `ship-feature` (Schritt 9) wartet über die Operation `pr-pruefstand-abwarten`; bei Rot bessert der `developer`-Subagent nach (Folgeauftrag „CI-Fehlschlag beheben") | Hauptsession (GitHub-Zugriff); Nachbesserung im **Subagenten** | — | immer, genau einmal je Lauf; nachgebessert wird nur, was sich lokal reproduzieren lässt, sonst hält der Ablauf an |
...
Der Abgleich mit `main` (5c und 7b) hält den Pull Request bis zur Freigabe mergebar: Ohne ihn steht Daniel regelmäßig vor einem inhaltlich fertigen PR, den er nicht freigeben kann, weil `main` zwischen Abzweig und Freigabe weitergelaufen ist. Er merged (nie Rebase, damit Review-Threads verankert bleiben) und holt dabei ausschließlich in den Remote-Tracking-Ref `origin/main`, damit `git diff origin/main...HEAD` in der Review-Phase genau die Änderungen des Branches zeigt (ADR [`decisions/0063-abgleich-mit-main-als-getestetes-lokales-skript-merge-statt-rebase.md`](../specs/decisions/0063-abgleich-mit-main-als-getestetes-lokales-skript-merge-statt-rebase.md), Vergleichsbasis und geschriebener Ref abgelöst durch ADR [`decisions/0075-abgleich-mit-main-fasst-den-lokalen-main-ref-nicht-mehr-an.md`](../specs/decisions/0075-abgleich-mit-main-fasst-den-lokalen-main-ref-nicht-mehr-an.md)). Der lokale `main`-Ref wird dabei nie angefasst — nur so läuft der Abgleich auch aus einem Arbeitsbaum unter `.claude/worktrees/` durch, während der Haupt-Checkout `main` ausgecheckt hat, und nur so bleibt jener Haupt-Checkout unversehrt.

Schritt 7c schließt die Lücke zwischen „alles gepusht" und Daniels Pflicht-Gate in Schritt 8: Vorher trug kein Ablaufschritt eine Aussage über den CI-Stand, und jede Session baute sich dafür ein eigenes Wegwerf-Skript. Gewartet wird an genau einem Punkt je Ablauf, nach dem letzten Push — jeder frühere Stand wird ohnehin überschrieben. Derselbe Schritt existiert im Entwurfspfad (`ship-entwurf`, dort Schritt 8); nachgebessert wird er allerdings in der Hauptsession, weil es dort keinen Subagenten gibt. Die Zeitgrenzen, das Ergebnisvokabular und die Zahl der Nachbesserungsrunden stehen ausschließlich im Katalogeintrag der Operation (ADR [`decisions/0091-ci-ergebnis-abwarten-und-begrenzt-nachbessern.md`](../specs/decisions/0091-ci-ergebnis-abwarten-und-begrenzt-nachbessern.md)).
