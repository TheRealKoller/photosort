...
Der Abgleich mit `main` (5c und 7b) hält den Pull Request bis zur Freigabe mergebar: Ohne ihn steht Daniel regelmäßig vor einem inhaltlich fertigen PR, den er nicht freigeben kann, weil `main` zwischen Abzweig und Freigabe weitergelaufen ist. Er merged (nie Rebase, damit Review-Threads verankert bleiben) und zieht dabei den lokalen `main`-Ref mit, damit `git diff main...HEAD` in der Review-Phase weiterhin genau die Änderungen des Branches zeigt (ADR [`decisions/0063-abgleich-mit-main-als-getestetes-lokales-skript-merge-statt-rebase.md`](../specs/decisions/0063-abgleich-mit-main-als-getestetes-lokales-skript-merge-statt-rebase.md)).
