...
# Modellwahl: sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2, int8-quantisierte
# ONNX-Variante - ueber onnxruntime+tokenizers ausgefuehrt, KEIN PyTorch/TensorFlow-Hub.
#
# Konkretes Asset-Paar: die bereits vorquantisierte ONNX-Variante von
# "Xenova/paraphrase-multilingual-MiniLM-L12-v2"
# (huggingface.co/Xenova/paraphrase-multilingual-MiniLM-L12-v2, MIT-Lizenz,
# Transformers.js-Community-Export DESSELBEN Basismodells, config.json bestaetigt
# "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2" als _name_or_path,
# hidden_size=384) - "onnx/model_int8.onnx" + "tokenizer.json".
#
...
class LabelEmbedderLike(Protocol):
    """Schmale, injizierbare Schnittstelle - bewusst SYNCHRON (reine, schnelle CPU-Inferenz auf
    kurzen Texten, kein Netzwerk, analog AestheticsModelLike.predict/FaceDetectorLike), erlaubt
...
def _mean_pool_and_normalize(
    ...
    """Reine, modellfreie Pooling-/Normierungsfunktion (Standard-Rezept fuer
    Sentence-Transformer-Modelle dieser Familie): mittelt die Token-Embeddings NUR ueber
    tatsaechlich attendierte Positionen (attention_mask == 1, Padding wird ignoriert), normiert das
    Ergebnis anschliessend L2 auf Einheitslaenge. Degenerierte Eingabe (komplett leere Maske, in
    der Praxis nie erwartet - jedes tokenisierte Label hat mindestens ein Token) liefert einen
    Nullvektor statt eines ZeroDivisionError/NaN, analog
    aesthetics.py::compute_aesthetics_score."""
...
def build_label_embedder() -> LabelEmbedderLike:
    """Baut das echte, lokale Text-Embedding-Modell: laedt den gepinnten Tokenizer + die gepinnte
    ONNX-Session (beide Assets oben, SHA256-verifiziert per eigenem Test, KEIN unverifizierter
    Bezug zur Laufzeit), tokenisiert einen kurzen Text, fuehrt eine InferenceSession-Inferenz aus
    und mean-poolt+normiert das Ergebnis (siehe _mean_pool_and_normalize). Laeuft WIE
