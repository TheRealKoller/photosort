...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("photo_id", "criterion_key", name="uq_criterion_score_photo_key"),
    )

    op.add_column("scoring_runs", sa.Column("gate_confirmed_at", sa.DateTime(), nullable=True))
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum(
                "RUNNING", "SUCCESS", "FAILED", name="scanstatus", native_enum=False, length=20
            ),
            nullable=False,
        ),
...
def downgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column(
            "category",
            sa.Enum(
                "LANDSCAPE", "DETAIL", "PEOPLE", name="photocategory", native_enum=False, length=20
            ),
            nullable=True,
        ),
    )
