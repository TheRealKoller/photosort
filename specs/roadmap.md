...
- **Auth-Implementierung** — [`specs/features/0006-auth.md`](./features/0006-auth.md) (Status: Accepted, geschärft im idea-sharpener-Gespräch vom 2026-07-20). Beim Schärfen von Spec 0002 am 2026-07-19 als fehlendes Prerequisite entdeckt. Login/JWT gemäß [`decisions/0003-auth-model.md`](./decisions/0003-auth-model.md) und [`decisions/0005-auth-implementation.md`](./decisions/0005-auth-implementation.md) (Argon2, PyJWT, Bearer+localStorage, Rate-Limiting via `slowapi`). Ist tatsächlich die erste sichtbare Oberfläche/erste Spec mit React Router + TanStack Query (kommt vor Spec 0005). Muss vor Spec 0002 stehen (deren Endpunkte brauchen `get_current_user`) und vor Spec 0005 (Login-Screen/geschützte Routen/Token-Handling) — Prerequisite für beide, höchste Priorität innerhalb "Jetzt".
...
| [0006](./features/0006-auth.md) | Auth-Implementierung | Accepted |

## Bekannte Abhängigkeiten

- **0001 → 0006 (Auth-Implementierung) → 0005 (Minimales Projekt-Frontend) → 0002:** Spec 0001 deckt nur das Backend ab (kein Frontend, kein Auth). Ursprünglich als zwei parallele Prerequisites zu 0002 eingeordnet; seit der Stakeholder-Entscheidung vom 2026-07-20 ist die Reihenfolge **sequenziell**: Spec 0005 setzt eine bereits umgesetzte Auth-Spec (0006) voraus (Login-Screen, geschützte Routen, Token-Handling sind Teil der Auth-Spec, nicht des Frontend-Grundgerüsts). Alle drei (0006, 0005, 0002) sind jetzt geschärft/akzeptiert; `developer` kann in dieser Reihenfolge umsetzen, siehe jeweils Abschnitt "Architektur / Umsetzung". Spec 0006 bringt zwei Muss-Kriterien mit direkter Sicherheitsrelevanz mit (Secret-Startup-Guard, Rate-Limiting via `slowapi`), da PhotoSort aus dem offenen Internet erreichbar ist.
