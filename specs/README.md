...
- `decisions/` — Architecture Decision Records (ADRs), verfasst vom `architect`-Agenten. Unveränderlich nach Annahme; eine spätere Änderung der Entscheidung erzeugt eine neue ADR, die die alte als "Superseded" markiert — bzw., wenn nur ein benannter Teil fällt, als "teilweise abgelöst" (siehe Abschnitt "ADR-Status" unten). Unveränderlich sind dabei der Abschnitt "Entscheidung", die Statuszeile und jeder Teil-Vermerk; "Kontext", "Begründung" und "Konsequenzen" dürfen redaktionell verdichtet werden, solange die Entscheidung selbst unberührt bleibt — ADRs im Status "Superseded" werden dabei gar nicht angefasst.
