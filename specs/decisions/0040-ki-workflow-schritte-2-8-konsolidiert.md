...
| Security | echt bedingt | Datei unter `backend/src/photosort/api/`, `.../opencloud/`; oder eine der Dateien `main.py`, `security.py`, `rate_limit.py`, `config.py`, `seed.py` unter `backend/src/photosort/`; oder neue Datei direkt unter `backend/src/photosort/`; oder Dependency-Datei (`backend/pyproject.toml`, `backend/uv.lock`, `frontend/package.json`, `frontend/package-lock.json`); oder `.env.example`; oder `.github/workflows/**`; oder Docker-Compose-Netzwerkkonfiguration; oder `frontend/src/auth/**` bzw. `frontend/src/api/client.ts`; oder `design/penpot/**`, `.claude/skills/penpot-design/**` bzw. `.claude/skills/penpot-entwurfsrunden/**` (ergänzt 2026-09-08 mit ADR 0065, der dritte Pfad 2026-09-10 mit ADR 0073 — ausführbare Nutzlast bzw. Anleitung für eine angemeldete Browsersitzung, von CI nicht nachstellbar; bewusst eng, `.claude/skills/**` bleibt außen vor); oder `.claude/skills/ship-entwurf/**` (ergänzt 2026-09-11 mit ADR 0077 — nicht der Werkzeugkanal ist hier der Grund, sondern die Richtung: der einzige Text, der festlegt, welche Namen aus einer privaten Design-Quelle in ein öffentliches, nicht zurücknehmbares Artefakt gelangen, und zugleich der einzige Pull-Request-Pfad ohne Perspektivenrunde); oder `scripts/nummern.py` (ergänzt 2026-09-14 mit ADR 0108 — auch hier ist die Richtung der Grund: das einzige Skript, das fremde Arbeitsbäume liest und in eine Migrationsdatei schreibt, mit acht eigenen Sicherheitsauflagen, von denen eine bei Verletzung Daniels Backend am Containerstart scheitern lässt; bewusst eng auf die eine Datei, `scripts/**` bleibt außen vor) |
...
Betroffen sind die fünf neuen `review-*`-Skills und der `review`-Orchestrator,
`.claude/skills/ship-feature/SKILL.md`, `.claude/agents/developer.md` (Anker-Definition wird die
einzige im Repository), die fünf Fachagenten-Dateien, `docs/ai-workflow.md`, `CLAUDE.md`,
`specs/architecture/0002-testkonzept.md` und `specs/diagrams/workflow-overview.d2`/`.svg`.
Kein Effekt auf `docs/architecture.md` und `docs/setup.md` — reine Prozess-/Workflow-Änderung.
