...
**Nachtrag (2026-09-05):** **Abschnitt 4** (Ersatz ist ein einzelnes, dünnes Helferscript `scripts/gh-board.py`, das zugleich einzige Board-Schreibstelle ist) ist durch ADR [`0057`](./0057-board-lebenszyklus-nativ-statt-eigenbau.md) abgelöst: Das Script wird samt seinen Tests gelöscht; an seine Stelle tritt kein neues Werkzeug, sondern eine Sammlung einzelner `gh`-Befehle in `.claude/skills/github-board/SKILL.md`, und drei der fünf Statusübergänge schreibt GitHub über native Projects-Workflows selbst. Die dort übernommenen Härtungsregeln aus ADR [`0017`](./0017-github-projects-v2-spec-sync.md) Abschnitt 5 verlieren ihren Code-Träger und sind in ADR 0057 als verbindliche Skill-Regeln neu gefasst (Freitext nur über Dateien, nur geschlossene Werte in einer Befehlszeile, kein `eval`). **Die Abschnitte 1–3 und 5 bleiben unverändert gültig**, insbesondere Spec-Nummer = Issue-Nummer. Diese ADR bleibt `Accepted`. Reiner Verweis, kein nachträgliches Editieren der ursprünglichen Entscheidung/Begründung unten.
