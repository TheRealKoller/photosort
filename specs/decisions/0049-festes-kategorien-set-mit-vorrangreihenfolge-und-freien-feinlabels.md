...
**Status:** Superseded — abgelöst durch ADR [`0091`](./0091-motive-mit-staerke-statt-hauptkategorie.md) (Motive mit Stärke statt einer Hauptkategorie). Der Kern dieser ADR entfällt vollständig: das feste Kategorien-Set, die deterministische Vorrangreihenfolge, der gemeinsame Kandidatenpool aus lokalen und remoten Signalen und `resolve_category` als Auswahlfunktion. **Weiter gelten** — dort übernommen und neu begründet: der Prompt entsteht ausschließlich aus der Registry und nie aus einem Literal, aus Datenbankinhalten oder aus einer früheren Modellantwort (Abschnitte 1/5), das geschlossene, zweistufig validierte Antwortschema (Abschnitt 5), die Feinlabels samt kanonischer Registry und Ähnlichkeitsauflösung (Abschnitt 6, unberührt), die Eingabevalidierung gegen ein geschlossenes Vokabular statt gegen eine foto-skopierte Menge (Abschnitt 8) und das Frontend ohne gespiegeltes Set (Abschnitt 9). Die beiden Teil-Vermerke unten betreffen ADR 0067 bzw. 0069, die mit derselben Umstellung ebenfalls abgelöst sind.
**Teilweise abgelöst:** **Abschnitt 5, dritter Aufzählungspunkt** („**Konfidenzen entfallen ersatzlos**") und der Halbsatz in **Abschnitt 8** („`score` entfällt dort ersatzlos … für lokale Kandidaten allein wäre die Spalte irreführend halb gefüllt") durch ADR [`0067`](./0067-modellkonfidenz-je-kategorie-anzeige-und-auswertung.md). Die Selbsteinschätzung des Modells wird ab dort je Kategorie festgehalten und angezeigt — die damalige Begründung („eine Zahl ohne Verwender wäre irreführender Ballast") bleibt gültig und wird dort ausdrücklich als Bedingung übernommen: Verwender benannt, Auswahl unberührt, kein erfundener Wert. **Alles Übrige dieser ADR bleibt unverändert in Kraft** — insbesondere das feste Set samt Definitionen und Vorrangreihenfolge (Abschnitte 1/2), der gemeinsame Kandidatenpool aus lokalen und remoten Signalen (Abschnitt 3), die lokal bestimmbare Teilmenge (Abschnitt 4), das geschlossene, zweistufig validierte Antwortschema im Übrigen (Abschnitt 5), die Feinlabels (Abschnitt 6 — deren eigene Konfidenz bleibt entfallen), Datenmodell und Migration (Abschnitt 7), die Override-Validierung gegen das geschlossene Vokabular (Abschnitt 8 im Übrigen) und das Frontend ohne gespiegeltes Set (Abschnitt 9). Insbesondere gilt unverändert: **die Kategorie bestimmt allein die Vorrangreihenfolge, nie ein Zahlenvergleich.** Deshalb `Accepted` und nicht `Superseded`; die Abstufung ist in [`../README.md`](../README.md) beschrieben.
**Teilweise abgelöst (zweiter Vermerk):** **der letzte Aufzählungspunkt der Konsequenzen** („Ein späterer Wechsel des Grundprinzips — … Mehrfachzuordnung eines Fotos zu mehreren Kategorien … braucht eine neue ADR, die diese hier als ‚Superseded' markiert") und der Begründungs-Halbsatz in **Abschnitt 7a** („1:1 statt 1:N, weil pro Foto genau eine Kategorie entsteht — das ist die Kernaussage der Story") durch ADR [`0069`](./0069-nebenkategorien-mehrfachzugehoerigkeit-und-konfidenzgewichtete-rangfolge.md). Ein Foto führt ab dort neben seiner Hauptkategorie weitere, allein aus der Modellkonfidenz abgeleitete Nebenkategorien. **Alles Übrige dieser ADR bleibt unverändert in Kraft** — insbesondere das feste Set samt Vorrangreihenfolge (Abschnitte 1/2), der eine Kandidatenpool (Abschnitt 3), die lokal bestimmbare Teilmenge (Abschnitt 4), das Antwortschema (Abschnitt 5), die Feinlabels (Abschnitt 6), Datenmodell und Migration im Übrigen (Abschnitt 7 — `photo_category_classifications` bleibt 1:1 zum Foto), die Override-Validierung (Abschnitt 8) und das Frontend ohne gespiegeltes Set (Abschnitt 9). Die **Hauptkategorie** wird weiterhin ausschließlich über die Vorrangreihenfolge bestimmt, nie über einen Zahlenvergleich; `resolve_category` bleibt buchstäblich unverändert. Die hier verlangte Form „als Superseded markieren" stammt aus der Zeit vor der in [`../README.md`](../README.md) beschriebenen Abstufung — ein volles `Superseded` wäre eine falsche Auskunft über das feste Set, das unverändert gilt.
**Datum:** 2026-08-30
