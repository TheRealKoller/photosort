...
**Status:** Superseded — abgelöst durch ADR [`0091`](./0091-motive-mit-staerke-statt-hauptkategorie.md) (Motive mit Stärke statt einer Hauptkategorie). Der Kern dieser ADR entfällt, weil es die Unterscheidung nicht mehr gibt, die sie trifft: ohne Hauptkategorie gibt es keine Nebenkategorie. Mit ihr fallen `photo_rankings.is_primary` samt der Invariante „genau eine Hauptzeile je (Lauf, Foto)", die Konfidenzschwelle `SECONDARY_CATEGORY_MIN_CONFIDENCE`, die konfidenzgewichtete Dämpfung des Sortierschlüssels (`CONFIDENCE_RANK_PENALTY`) und `category_key` als Dimension des Partitionsschlüssels — die Partition ist dort allein das Event, und ein Foto steht pro Lauf wieder in genau einer Zeile. **Weiter gilt**, dort übernommen und tragend: eine Mehrfachaussage über ein Foto gehört in persistierte Zeilen und nicht in eine zur Lesezeit expandierte JSON-Struktur (Punkt 1).
