...
**Status:** Accepted
**Teilweise abgelöst:** **Abschnitt 3** („Der lokale `main`-Ref wird mitgezogen, sonst lügt der Review-Diff") und der daran hängende dritte Punkt der Konsequenzen („`git diff main...HEAD` bleibt an allen sechs Fundstellen korrekt … Wer den `fetch` je auf `git fetch origin` ohne Refspec vereinfacht, bricht die Review-Runde still") durch ADR [`0075`](./0075-abgleich-mit-main-fasst-den-lokalen-main-ref-nicht-mehr-an.md). Der Abgleich holt ab dort mit `+refs/heads/main:refs/remotes/origin/main`, merged `origin/main` und fasst `refs/heads/main` nie mehr an; die Vergleichsbasis der Review-Phase ist `origin/main...HEAD`. Grund: `git fetch origin main:main` verweigert gemessen den Dienst (Exit 128), sobald `main` in irgendeinem Arbeitsbaum desselben Repositoriums ausgecheckt ist — der Normalfall für Hintergrund-Läufe unter `.claude/worktrees/`. **Alles Übrige dieser ADR bleibt unverändert in Kraft** — insbesondere der Abgleich als getestetes lokales Skript ohne Argumente (Abschnitt 1), Merge statt Rebase mit `--no-ff` und fester einzeiliger `chore:`-Nachricht (Abschnitt 2), die Ausgänge samt gerechnetem No-Op (Abschnitt 4), die Rollenteilung mit ihren beiden Ankern (Abschnitt 5), die zwei Aufrufzeitpunkte (Abschnitt 6) und der gesamte Nachtrag. Deshalb `Accepted` und nicht `Superseded`; die Abstufung ist in [`../README.md`](../README.md) beschrieben.
**Datum:** 2026-09-08
