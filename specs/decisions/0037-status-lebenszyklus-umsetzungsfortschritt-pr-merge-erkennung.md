...
**Status:** Superseded — abgelöst durch ADR [`0057`](./0057-board-lebenszyklus-nativ-statt-eigenbau.md) (der Board-Lebenszyklus wird nativ: GitHub schreibt, was GitHub erkennt, `gh-board.py` entfällt). Abgelöst sind Abschnitt 1 (sechs Board-Werte inklusive `Todo`), Abschnitt 3 (`In Progress` beim Aufrufer des `developer`), Abschnitt 4 (`Review` in `ship-feature`), Abschnitt 5 (keine native Board-Automatisierung, Merge-Erkennung im eigenen Code) sowie die Abschnitte 6 und 7; Abschnitt 2 (Baseline/Override aus dem Datei-Status) war bereits durch ADR [`0043`](./0043-spec-nummer-gleich-issue-nummer-sync-tool-entfaellt.md) gegenstandslos. Der Lebenszyklus umfasst seither genau fünf Werte (`Unrefined` → `Ready` → `In Progress` → `Review` → `Done`), und drei der fünf Übergänge löst GitHub selbst aus. Der ältere Nachtrag weiter unten stammt aus der Zeit davor und hält fest, diese ADR bleibe `Accepted`; maßgeblich ist diese Statuszeile.
**Datum:** 2026-08-27
