...
**Status:** Superseded — abgelöst durch ADR [`0057`](./0057-board-lebenszyklus-nativ-statt-eigenbau.md). Der **Befund** zur Remote-Grenze bleibt unverändert gültig (die Session-Zwischenschicht bedient GraphQL nur für einen festen Satz von PR-Operationen, Projects V2 spricht ausschließlich GraphQL) und ist in ADR 0057, Abschnitt 7 fortgeführt — dort schrumpft die Grenze von vier Board-Schritten auf zwei, weil `Review` und `Done` auf GitHubs Servern entstehen. Die **Vorabmessung** `capabilities` samt der auf sie gestützten Auslass-Logik entfällt dagegen ersatzlos: Übrig sind zwei Schritte aus je einem einzelnen Befehl, deren Versuch nicht mehr kostet als ihre Messung.
