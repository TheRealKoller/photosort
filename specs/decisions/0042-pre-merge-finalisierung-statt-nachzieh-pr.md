...
**Nachtrag (2026-09-05):** **Abschnitt 2** (expliziter Finalisierungsmodus im Werkzeug) und **Abschnitt 4** (ein vorgezogener Board-Wert `Done` ist zulässig, weil das Board selbstheilend ist) sind durch ADR [`0057`](./0057-board-lebenszyklus-nativ-statt-eigenbau.md) abgelöst. Das Werkzeug entfällt; die `**Status:**`-Zeile der Spec-Datei schreibt `ship-feature` ab jetzt als gewöhnliche lokale Dateiänderung. Ein vorgezogenes `Done` gibt es nicht mehr: `Done` entsteht ausschließlich daraus, dass der Merge das Issue über das Closing-Keyword schließt, und keine Session schließt vor dem Merge noch ein Issue. **Die Abschnitte 1, 3 und 5 bleiben unverändert gültig** — der Regelweg ist weiterhin die Finalisierung *vor* dem Merge im Feature-PR selbst, gebündelt mit dem letzten Push. Diese ADR bleibt `Accepted`. Reiner Verweis, kein nachträgliches Editieren der ursprünglichen Entscheidung/Begründung unten.
