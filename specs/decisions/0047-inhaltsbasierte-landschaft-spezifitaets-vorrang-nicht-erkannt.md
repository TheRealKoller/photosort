...
**Teilweise abgelöst:** ausschließlich **Punkt 7** (`category_diff.py` als Vorher/Nachher-Werkzeug zweier Läufe), durch ADR [`0091`](./0091-motive-mit-staerke-statt-hauptkategorie.md) — das Werkzeug vergleicht die Kategoriezuordnung zweier Läufe und hat ohne Kategorien keinen Gegenstand mehr; es entfällt dort ersatzlos. **Punkt 1 (`landschaft`-Kriterium aus der Szenen-Klassifikation, `content_landscape` als reines Ranking-Signal) gilt unverändert weiter** und trägt dort die lokale Stärke der Motive `landschaft` und `bauwerk_sehenswuerdigkeit`, ebenso Punkt 5 (`is_landmark_candidate` als Vorfilter an der Vertrauensgrenze) und Punkt 6 (kein rückwirkender Eingriff in ältere `PhotoRanking`-Zeilen).
**Datum:** 2026-08-30
