...
**Status:** Accepted
**Teilweise abgelöst:** **Abschnitt 4, erster Satz** („Der Board-Zugriff sind ab jetzt einzelne `gh`-Befehle in den Skill-Dateien") und **Abschnitt 7** (Umfang der Remote-Grenze) durch ADR [`0059`](./0059-ein-ort-fuer-jeden-github-zugriff-wege-in-fester-reihenfolge.md). Ein GitHub-Zugriff ist ab dort eine Operation mit mehreren Zugangswegen in fester Reihenfolge (`mcp` vor `gh`), gesammelt im Skill `github-access`; die Remote-Grenze schrumpft von „Board plus alle Issue-Schritte" auf die vier Board-Operationen. **Alles Übrige dieser ADR bleibt unverändert in Kraft** — insbesondere der fünfwertige Lebenszyklus (Abschnitt 1), die Auslöser-Tabelle mit den drei nativen Übergängen (Abschnitt 2), `Done` heißt „vom Board" (Abschnitt 3), der Ersatz der vier Werkzeug-Prüfungen (Abschnitt 5), das Fehlschlag-Muster samt `## Lokal nachzuholen` (Abschnitt 6), der PR-ohne-Merge-Rückfall (Abschnitt 8) und die Festlegung, dass an die Stelle des gelöschten Werkzeugs kein neues tritt. Deshalb `Accepted` und nicht `Superseded`; die Abstufung ist in [`../README.md`](../README.md) beschrieben.
**Datum:** 2026-09-05
...
4. **Ein ausgebliebener nativer Übergang wird bemerkt.** Mit drei von fünf Übergängen auf GitHubs Seite dreht sich die Richtung des Fehlers um: Bisher war der Fehlerfall ein *falscher* Wert und fiel dadurch auf; ein versehentlich deaktivierter Workflow schreibt dagegen **gar nichts**, und eine Karte, die auf `In Progress` liegen bleibt, ist von einer Karte, an der gerade gearbeitet wird, nicht zu unterscheiden. Der Zustand der Workflows ist über GraphQL zwar **lesbar** (Abschnitt 1), aber nicht schreibbar — und lesen könnte ihn ohnehin nur eine Session mit Token, nie die CI. Zurückgelesen wird deshalb nicht der Workflow, sondern sein **Ergebnis**: der Wert der Karte. Das ist der stärkere Nachweis, weil er auch den Fall trifft, in dem der Workflow zwar läuft, aber auf einen anderen Zielwert konfiguriert wurde. `ship-feature` liest den Board-Wert nach dem Eröffnen des Pull Requests **einmal zurück** — mit dem Lesebefehl aus Abschnitt 4, der ohnehin in der Sammlung steht — und führt ein ausgebliebenes `Review` unter `## Lokal nachzuholen` auf. Steht dabei noch der alte Wert, wird nach kurzer Wartezeit **ein zweites Mal** gelesen, bevor daraus ein Befund wird: GitHub verarbeitet die Verknüpfung asynchron, und ein unmittelbar nach `gh pr create` gelesener Wert meldete sonst systematisch einen Fehlschlag, den es nicht gibt. Im Regelfall bleibt es bei einem zusätzlichen Befehl pro Story, der zugleich die Probe aus Abschnitt 2 dauerhaft macht, statt sie einmalig am Pull Request dieser Story zu belassen. Am 2026-09-05 gegen „als bekannte Lücke akzeptieren" abgewogen und von Daniel entschieden.
