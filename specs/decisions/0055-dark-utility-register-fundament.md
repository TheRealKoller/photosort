...
**Teilweise abgelöst:** durch [`0065-penpot-als-design-quelle-rangfolge-umgekehrt.md`](./0065-penpot-als-design-quelle-rangfolge-umgekehrt.md). Betroffen ist ausschließlich der Satz der Begründung „Board-Werte sind eine Vorlage, kein Gesetz" **in seiner vorwärtsgerichteten Wirkung**: Die Design-Quelle ist seit ADR 0065 für die Gestaltung normativ, und sie heißt Penpot statt Figma. **Alle acht Entscheidungspunkte gelten unverändert weiter**, insbesondere Punkt 4 — die Kontrast-Untergrenze schlägt den Wert der Design-Quelle; ADR 0065 Abschnitt 4 schreibt genau diese Regel fort und wendet sie auf Penpot an. Kein `Superseded`: Der Kern dieser Entscheidung (dunkel als einziges Schema, Token-Architektur, Konfliktauflösung, Kategorie-Chips, Symbolsatz, Zielflächen) fällt nicht.

**Teilweise abgelöst:** in **Punkt 4b** ausschließlich die kompensierende Maßnahme „gedämpfte Bildfläche (Deckkraft/Filter **nur** auf dem Vorschaubild)", durch ADR [`0112`](./0112-keine-bildflaeche-wird-gedaempft.md). Auf einer Bildfläche liegt ab dort keine Dämpfung mehr, in keiner Ansicht: Überall dort, wo gedämpft wurde, wird inzwischen das Motiv selbst beurteilt, und Deckkraft auf der Bildfläche mischt gegen den Seitengrund. **Alles Übrige von Punkt 4b gilt unverändert** — die harte Bindung von `--text-disabled` an `disabled`/`aria-disabled`, der Dateiname in `--text-muted` statt `--text-disabled`, der Wegfall der Board-Angabe „ganze Karte Deckkraft 40 %" (ab ADR 0112 schärfer als zuvor) und die Mehrfachcodierung aus Textbadge, eigenem Symbol und Durchstreichung, die die Dämpfung nie enthielt. Die übrigen Entscheidungspunkte sind unberührt. Deshalb `Accepted` und nicht `Superseded`; die Abstufung ist in [`../README.md`](../README.md) beschrieben.
...
**Entscheidung:** Die bisherige 44px-Regel wird von einer **Größen**regel zu einer **Trefferflächen**regel. Bedienelemente werden auf die kompakten Board-Maße gebracht (Standard-Schaltfläche 32px hoch statt 44px), und die Trefferfläche wird über ein transparentes Pseudo-Element auf mindestens 44×44px erweitert — **zwei** Utilities (`@layer utilities`), die von Button, Switch, Checkbox und Icon-Button gemeinsam genutzt werden. *Diese Stelle sagte ursprünglich „eine einzige Utility“; die Einschätzung wird hiermit korrigiert.* Bei der Umsetzung hat sich gezeigt, dass die erste der vier Aufspannungsregeln aus dem UI/UX-Abschnitt der Spec 0320 („nur auf der kurzen Achse aufspannen“) mit einer einzigen Utility nicht umsetzbar ist: `tap-target` spannt nur vertikal auf (eine beschriftete Schaltfläche ist breit genug), `tap-target-square` beidachsig (Symbol-Schaltflächen, Kontrollkästchen). Eine beidachsige Aufspannung an einer breiten Schaltfläche erzeugte genau die breiten unsichtbaren Flächen, die dieselbe Regel verbietet — die Aufteilung ist damit keine Ausweitung der Entscheidung, sondern ihre Bedingung.
