...
**Status:** Superseded — abgelöst durch ADR [`0091`](./0091-motive-mit-staerke-statt-hauptkategorie.md) (Motive mit Stärke statt einer Hauptkategorie). Die tragende Grenze dieser ADR — „die Selbsteinschätzung des Modells berührt die Kategorieauswahl nicht" (Punkt 1) — ist dort **bewusst aufgegeben**: die Zahl je Motiv ist ab dort die Auswahlgrundlage selbst, tragbar nur, weil sie stets innerhalb eines Motivs verglichen wird und nie zwischen zwei Motiven. Mit ihr fallen Punkt 3 (Neutralwert „keine Angabe", nie `0.0` — eine nicht genannte Stärke ist dort die Aussage „nicht zu sehen") und Punkt 5 (Auswertung nach der Modell-Kategorie). **Weiter gelten**, dort übernommen: die Zahl hängt am Schlüssel und nicht an einer Position oder an der Herkunft (Punkt 2), und die Auswertung läuft in SQL statt über einen JSON-Zugriff, den SQLite und PostgreSQL unterschiedlich schreiben (Punkt 4). Der Teil-Vermerk unten betrifft ADR 0069, die mit derselben Umstellung ebenfalls abgelöst ist.
**Teilweise abgelöst:** **Punkt 1** in seiner Reichweite und **Punkt 6** vollständig, durch ADR [`0069`](./0069-nebenkategorien-mehrfachzugehoerigkeit-und-konfidenzgewichtete-rangfolge.md). Punkt 1 gilt ab dort für die **Hauptkategorie**: `resolve_category` bleibt buchstäblich unverändert, ohne Konfidenz-Parameter, und der Invariantentest bleibt gültig — die Konfidenz entscheidet aber über die Zugehörigkeit zu **Neben**kategorien und über die Reihenfolge innerhalb einer Kategorie. Punkt 6 fällt in beiden Aussagen: es gibt seither eine feste Schwelle im Backend (`categories.py::SECONDARY_CATEGORY_MIN_CONFIDENCE`, eine Aussage über die Taxonomie, nicht über „unsicher") und eine konfidenzgewichtete Reihenfolge — allerdings als auf `0,15` begrenzte Gewichtung des bestehenden Rang-Scores, nicht als Sortierung *nach* Konfidenz; die dort abgelehnte Wirkung („nimmt der Ansicht die Aussage, für die es sie gibt") tritt damit nicht ein. Der clientseitige „unsicher"-Filter der Kuratierung bleibt unverändert eine Konstante der Oberfläche. **Punkt 2 (die Zahl hängt am Schlüssel), Punkt 3 (kein Wert wird erfunden), Punkt 4 (zwei Spalten), Punkt 5 (Auswertung nach der Modell-Kategorie) und Punkt 7 (tolerantes Antwortschema) bleiben unverändert in Kraft**; die Abstufung ist in [`../README.md`](../README.md) beschrieben.
**Datum:** 2026-09-09
