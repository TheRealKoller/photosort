...
**Status:** Accepted
**Teilweise abgelöst (1):** ausschließlich der **Partitionsschlüssel** — überall dort, wo diese ADR die Partition als `cluster_key`/`event_id` **× `category_key`** führt, und damit der Parameter `category_key` des Vorrats-Endpunkts —, durch ADR [`0091`](./0091-motive-mit-staerke-statt-hauptkategorie.md). Die Partition ist ab dort allein das Event; ein Foto steht pro Lauf in genau einer Zeile.
**Teilweise abgelöst (2):** ausschließlich die **Auswahlregel** „die besten N je Partition, N als Leseparameter" (Entscheidung 1, erster Absatz) und der Query-Parameter `top_n_per_event`, durch ADR [`0097`](./0097-auswahl-mit-richtwert-kontingente-je-event-und-motivgefuehrte-vergabe.md). An ihre Stelle tritt ein persistierter Vorschlag mit Kontingenten je Event und motivgeführter Vergabe; damit entfällt auch die dort mitgeführte Aussage, die Kuratierungsantwort wachse nicht über `top_n <= 10` je Partition hinaus.
**Unverändert gültig** nach beiden Ablösungen: die stabile Auswahl ohne Backfill (ein verworfenes Foto bleibt an seiner Position, es rückt keines nach), der sichtbare und einsehbare Kandidatenvorrat samt Seitenabruf, `curation_position` als alleinige Auskunft darüber, wo die Kuratierung ein Foto zeigt, und die Auflage am Antwortaufbau, einen Zwischenspeicher/`ETag` über `no-store` hinaus nur mit dem Nutzer im Schlüssel zu führen. Die Abstufung ist in [`../README.md`](../README.md) beschrieben.
**Datum:** 2026-09-09
