...
**Teilweise abgelöst:** [`decisions/0068-klassifizierungslauf-vier-teilschritte-und-laufeigene-cloud-bilanz.md`](./0068-klassifizierungslauf-vier-teilschritte-und-laufeigene-cloud-bilanz.md) löst **Punkt 3, Absatz „Kein FK zwischen den beiden Run-Tabellen"** ab: `criterion_scoring_runs` bekommt einen nullable Fremdschlüssel auf `remote_category_classification_runs`, weil die laufbezogene Kostenbilanz eine Zuordnung braucht, die einen Geldbetrag trägt (die Sortier-Heuristik „jüngste Remote-Zeile des Projekts" ordnet einem Lauf ohne Cloud-Phase die Zahlen des Laufs davor zu). Ebenfalls erweitert, aber **nicht** abgelöst: der `ClassificationPhase`-Wertebereich aus Punkt 1 (zwei weitere Teilschritte `landmark`/`ranking`, gleiche Semantik, `NULL` unverändert „läuft nicht mehr") und die Vorab-Schätzung aus Punkt 5 (getrennt ausgewiesene Anteile, `null` statt `0` für den vor dem ersten Lauf nicht schätzbaren Landmark-Anteil). Alles Übrige dieser ADR gilt unverändert weiter — insbesondere die Verkettung selbst, das Konjunktions-Gate aus Punkt 2, `criterion_scoring_runs` als Lauf-Datensatz des Gesamtlaufs samt Verzicht auf eine vierte Run-Tabelle und die laufweite `cloud_error_message` aus Punkt 4. Der Status bleibt deshalb `Accepted` (siehe [`../README.md`](../README.md), Abschnitt „ADR-Status").
