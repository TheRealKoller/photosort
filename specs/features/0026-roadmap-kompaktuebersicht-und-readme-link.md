...
**Nachtrag (2026-08-08, requirements-engineer-Konsultation auf Wunsch von Daniel):** Die mit dieser Spec eingeführte D2-Grafik (AK1/AK3/AK4) wurde in einer späteren Iteration am selben Tag wieder aus `specs/roadmap.md` entfernt — Daniel wollte stattdessen die bestehende Tabelle "Status auf einen Blick" nach Priorität in mehrere Tabellen aufgeteilt sehen (offene Specs nach Priorität sortiert oben, implementierte Specs unten), ohne begleitende Grafik. `specs/diagrams/roadmap-overview.d2`/`.svg` wurden gelöscht, da nach dem Rückbau nirgends mehr referenziert. Die übrigen Akzeptanzkriterien dieser Spec (Tabelle an den Dateianfang verschoben, README-Verlinkung) bleiben unverändert erfüllt. Diese Spec bleibt als historisches Dokument `Implemented` stehen (beschreibt korrekt, was zum Zeitpunkt der Umsetzung gebaut und akzeptiert wurde) statt rückwirkend geändert oder auf `Superseded` gesetzt zu werden — für die Grafik selbst existiert keine Nachfolge-Spec, nur eine spätere, kleinere Anpassung von `roadmap.md` im selben Themenbereich.
