...
- [ ] Der CI-Job ist **blockierend** (kein `continue-on-error`, kein Pfadfilter — ein Backend-Change kann die Oberfläche brechen). Dazu gehört die zweite, im Repository unsichtbare Hälfte: `e2e` steht seit dem 2026-09-05 in der Liste der für `main` erforderlichen Checks des Branch-Schutzes — ohne sie wäre ein roter Job nur ein rotes Häkchen, kein Merge-Hindernis (siehe `specs/architecture/0002-testkonzept.md`, Abschnitt „‚Blockierend' ist zur Hälfte eine Einstellung außerhalb des Repositoriums"). Jeder Layout-Spec belegt bei seiner Einführung einen **roten Lauf im PR** (Eigenschaft mutwillig kaputt gemacht); ohne diese Klausel ist „schlägt bei einem gebrochenen Ablauf fehl" nicht nachweisbar, sondern nur behauptet.
- [ ] Die bestehende Festlegung des Testkonzepts, kein dediziertes E2E-Test-Setup zu betreiben, wird bewusst und sichtbar abgelöst — mit dokumentierter Begründung in ADR [`0058`](../decisions/0058-browsergestuetzte-oberflaechenpruefung.md) und im selben PR im Testkonzept nachgezogen, an beiden Stellen („Was bewusst nicht getestet wird" und Frontend-Sektion „E2E-Ebene").
...
- **`e2e/`** (neues Top-Level-npm-Paket, neben `backend/`, `frontend/`, `scripts/`): `@playwright/test`, eigene `package.json`/`package-lock.json`/`tsconfig.json`. Bewusst nicht in `frontend/` eingehängt — `vitest` und `@playwright/test` kollidieren über `test`/`expect`, der `frontend`-CI-Job soll ohne Browser-Download bleiben, und E2E läuft in Node statt im DOM-Kontext.
  ...
  - `e2e/tests/*.spec.ts` — acht Specs, siehe Teststrategie.
...
- **`docker-compose.e2e.yml`** (neu, Root, additives Overlay analog `docker-compose.demo.yml`): eigener Compose-Projektname `photosort-e2e` (M5), Ports auf `127.0.0.1` per **`ports: !override`** (Compose *ergänzt* `ports`-Listen beim Überlagern — eine naive Überlagerung ließe die offene Bindung bestehen), **jede** Variable des Produktivstacks als expliziter Wert (M4, nicht nur die in ADR 0058 zunächst aufgezählten), kein OpenCloud.
...
**E2E (`e2e/`, Playwright/Chromium):** acht Specs — `grid-columns`, `sticky-header`, `popover-position`, `tap-targets`, `no-horizontal-scroll`, `empty-and-error-states`, `login`, `toolchain`. Die ersten sieben prüfen die **Anwendung**; der achte prüft die Zusagen des **Prüfwerkzeugs selbst** (Verlässlichkeitsregime, Viewport-Projekte, Ziel-Allowlist der Ad-hoc-Werkzeuge, Bindung der Doku an den Code) und ist bei der Umsetzung hinzugekommen, weil diese Zusagen sonst nur in einer Konfigurationsdatei stünden, die man schnell und leise ändert. Die vollständige Tabelle inklusive „was ihn bei kaputtem Layout rot macht" steht in `specs/architecture/0002-testkonzept.md`.
...
- **Werkzeug: Playwright allein, kein MCP-Server in dieser Stufe** — von Daniel am 2026-09-05 bestätigt (ADR 0058, Punkt 1). Ein Werkzeug für Ad-hoc-Blick und CI-Prüfsatz; ein Skript, mit dem ein Fehler reproduziert wurde, wird unverändert zum CI-Spec. Ein MCP-Server kann später ergänzt werden, wenn sich Bedarf zeigt.
- **Der automatisierte Lauf kommt ohne den OpenCloud-Container aus** — von Daniel am 2026-09-05 bestätigt (ADR 0058, Punkt 5). Das ungepinnte Rolling-Image wäre eine Sprunghaftigkeitsquelle und stünde damit direkt gegen das Akzeptanzkriterium „ein Fehlschlag ist ein verlässliches Signal".
- **Der Demo-Zustands-Seeder liegt im Backend-Paket, nicht in `scripts/`** — von Daniel am 2026-09-05 bestätigt (ADR 0058, Punkt 4). Kein zweites Abbild von Datenmodell und Thumbnail-Cache-Schlüssel; Preis ist ein per M1–M3 geschützter Demo-Codepfad im Produktiv-Image.
...
- **Alle vier Fachkonsultationen sind gelaufen, keine übersprungen:** `architect` (Schritt 1, ADR 0058 angelegt), `ux-ui-designer` (Schritt 2, Ergebnis „nicht relevant" mit Begründung), `test-engineer` und `security-engineer` (Schritt 3, beide mit Nachzug im jeweils eigenen lebenden Dokument).
- **ADR 0058 Punkt 6 wurde nach der Security-Konsultation nachgeschärft:** die dort zunächst aufgezählte Variablenliste war unvollständig (M4) und ist jetzt als Mindestmenge samt CI-Nachweis formuliert; der eigene Compose-Projektname (M5) ist als Punkt 6d ergänzt.
