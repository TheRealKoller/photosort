...
**Status:** Implemented ([PR #188](https://github.com/TheRealKoller/photosort/pull/188) Strang 1, [PR #189](https://github.com/TheRealKoller/photosort/pull/189) Strang 2)
**Erstellt:** 2026-08-22
...
- [x] **AK11 (Vier-Fragen-Klassifikation je Fundstelle, nicht pro Datei):** Für jede in den 11 Dateien gefundene ADR-/Spec-Nummer-Referenz (Auffindung via `grep -noE '(ADR|Spec)[[:space:]]*\[?`?[0-9]{4}' <Datei>`) wird explizit dokumentiert, welcher der vier Fälle zutrifft (funktional nötig → behalten / Bündelfall → nur Begründungsklausel streichen / Status-Fortschritts-Ausnahme → behalten / reine Herkunftsbegründung → entfernen). Kein pauschales "Datei X ist jetzt bereinigt" ohne Fundstellen-Liste.
- [x] **AK12 (Regel-Substanz bleibt vollständig, Satz bleibt grammatisch intakt):** Nach Entfernen einer Herkunfts-/Begründungsklausel ist der verbleibende Satz weiterhin vollständig, eigenständig verständlich und frei von verwaisten Satzfragmenten (z.B. kein Komma/Bindestrich, der ins Leere zeigt).
- [x] **AK13 (Bündelfall, konkret belegt):** Wo funktionale Anweisung und Begründungsklausel im selben Satz stehen, wird nur die Begründungsklausel gestrichen. Beispiel: `architect.md` ("...ruft dich daraufhin auf (nicht mehr `developer` selbst — siehe ADR 0024)") → "...ruft dich daraufhin auf (nicht mehr `developer` selbst)"; die funktionale Aussage bleibt unverändert. Gleiches Muster im neuen `CLAUDE.md`-Bullet zum PR-Workflow.
- [x] **AK14 (funktionale ADR-Referenzen bleiben trotz Nummer):** Referenzen, bei denen die Datei selbst eine Pflegepflicht gegen die ADR formuliert (z.B. `ship-feature/SKILL.md` Schritt 3: "bei jeder künftigen Änderung zuerst in ADR 0014, dann hier synchron aktualisieren"), bleiben unverändert — funktional, keine Historie.
- [x] **AK15 (Status-Ausnahme in `design-system/SKILL.md` explizit unangetastet):** Alle "(noch nicht implementiert, vorgesehen für Spec NNNN)"/"(implementiert mit Spec NNNN, Referenz ...)"-Vermerke bleiben wortgleich erhalten.
- [x] **AK16 (Statusmarker vs. Herkunftszitat im selben Klammerausdruck sauber getrennt):** Wo ein Klammerausdruck sowohl Status als auch Herkunft mischt (Beispiel `CLAUDE.md`, Issue-Freigabe-Policy-Absatz: "vorbereitet mit Spec 0007/ADR 0007 ..., technisch noch nicht durchgesetzt, da die Automatisierung selbst noch nicht existiert"), wird nur der Herkunftsteil gestrichen, der Statusteil bleibt.
- [x] **AK17 (keine neue Herkunfts-Referenz im neuen CLAUDE.md-Bullet selbst):** Der neue Konventions-Bullet in `CLAUDE.md` (Abschnitt "Konventionen") formuliert die Regel selbstständig, ohne auf Inbox 0030/Spec 0053 als Begründung zu verweisen.
- [x] **AK18 (Verifikationsmethode als Pflicht-AK):** Abschließender `grep -noE '(ADR|Spec)[[:space:]]*\[?`?[0-9]{4}' .claude/agents/*.md .claude/skills/*/SKILL.md CLAUDE.md` (nach der Änderung) darf nur noch Treffer liefern, die laut AK14/AK15 explizit als "bleibt" klassifiziert wurden — jeder andere verbleibende Treffer ist ein Muss-Fix-Finding im Review.
...
### Strang 2 — Review-Ergebnis (PR 2)

- **Muss-Fix behoben:** In `.claude/skills/ship-feature/SKILL.md` hing nach dem Entzitieren der Einleitungszeile (Zeile 61) die Tabellenüberschrift "Ab dieser ADR maßgebliche Zeile" ohne Bezugsobjekt in der Luft ("dieser ADR" verwies zuvor auf das nun gestrichene ADR-0024-Zitat) — ein verwaistes Satzfragment (AK12), zusätzlich ein per AK18-Grep gefundener, in der Spec nirgends als "bleibt" klassifizierter Treffer ("ADR 0014" in der linken Spaltenüberschrift). Von `architect` (Standard-Modell) unabhängig gefunden, von `developer` durch Umformulierung beider Spaltenköpfe auf reine Funktion behoben (`| Bisherige Zuständigkeit | Jetzige Zuständigkeit | Modell (unverändert) |`), Tabellenzeilen selbst unverändert.
- **`test-engineer` (Standard-Modell):** keine Muss-Fixes; stufte dieselbe Tabellenüberschrift als "grenzwertig, aber vertretbar" ein — die AK18-Grep-Methode kann eine wortlose Bezugslücke ("dieser ADR" ohne Ziffer) strukturell nicht erkennen, deshalb hier `architect`s Fund als maßgeblich übernommen, nicht als Widerspruch gewertet.
- **Gemeldeter Fund geprüft und verworfen:** `requirements-engineer` (Haiku) meldete einen Muss-Fix zu `ship-feature/SKILL.md` Zeile 61 selbst ("identische Kopie von ADR 0024 Teil 6" sei fälschlich als reine Herkunftsklausel statt als AK14-Pflegepflicht entfernt worden). Der Orchestrator hat das gegen `git show main:...` und den AK14-Beispieltext der Spec selbst verifiziert: anders als die in AK14 explizit genannte Zeile 47 ("bei jeder künftigen Änderung zuerst in ADR 0014, dann hier synchron aktualisieren") enthielt Zeile 61 keine vergleichbare Pflegepflicht-Formulierung, nur eine Herkunftsangabe — Entfernung war korrekt, kein Revert. Von `test-engineer` in seinem eigenen Review unabhängig bestätigt.
- **`security-engineer`/`ux-ui-designer` laut Trigger-Tabelle (ADR 0014/0024) nicht gestartet:** reiner Instruktionsdatei-Diff ohne Frontend-/Auth-/Datenmodell-Bezug.
- **Kein Copilot-Review angefordert:** PR ändert ausschließlich `.claude/agents/*.md`, `.claude/skills/*/SKILL.md` und `CLAUDE.md`, keine Code-Datei (analog Strang 1/PR #188 und Spec 0025/PR #56).
- **Zwei Standard-Review-Läufe scheiterten zunächst an einem Sitzungslimit** ("You've hit your session limit"), ohne verwertbares Ergebnis — nach Ablauf des Limits erneut frisch gestartet (nicht per SendMessage fortgesetzt, da die ursprünglichen Subagenten-Fenster nicht mehr erreichbar waren) und danach vollständig durchgelaufen.
