...
- [ ] **(geschärft)** Keine Datei unter `.claude/**` außer `.claude/skills/github-access/SKILL.md`
      ...
      *Warum geschärft:* „Befehl" gegen „bloße Erwähnung" abzugrenzen ist eine Auslegungsfrage
      ...
      entscheidbar und deckt sich mit ADR 0061: Erlaubt bleibt außerhalb des Katalogs
...
> **Gewählter Ansatz** (ADR [`0061`](../decisions/0061-ein-ort-fuer-jeden-github-zugriff-wege-in-fester-reihenfolge.md)):
...
- **Generisch `\bgh [a-z][a-z-]*` statt der Vierer-Liste.** `gh auth status` ist exakt die von
  ADR 0061 verbotene Vorabmessung und entginge einer Aufzählung aus `gh issue|gh pr|gh
...
- `architect` konsultiert (Schritt 1): ADR [`0061`](../decisions/0061-ein-ort-fuer-jeden-github-zugriff-wege-in-fester-reihenfolge.md)
...
- `test-engineer` konsultiert (Schritt 3): Testkonzept um die Sektion „Erweiterung für ADR 0061"
...
- `security-engineer` konsultiert (Schritt 3): Sicherheitskonzept um den Angriffsflächen-Abschnitt
  „Ein zweiter Zugangsweg zu GitHub" ergänzt. Vier Punkte, die in ADR 0061 fehlten, sind
...
- **Abweichung von der Erlaubnisstufen-Tabelle in ADR 0061:** Die fünf Perspektiven-Skills tragen
