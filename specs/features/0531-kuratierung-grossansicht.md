...
- Der Modal-Hook gibt den Fokus in seiner Aufräumphase zurück, der Effekt der Seite läuft danach und setzt sich damit durch. Auf dieselbe Reihenfolge baut schon `AlbumDraftPage::handleExchange`.

### Komponenten

| Datei | Änderung |
|---|---|
| `frontend/src/lib/useModalDialog.ts` (neu) | Wird **ohne Verhaltensänderung** aus `components/ui/dialog.tsx` herausgelöst. Enthält `showModal()`/`close()`, die Fokusfalle, die Absprache zwischen Esc und `cancel`, `lockBodyScroll`, die Fokus-Rückgabe an das vorher fokussierte Element und den Erstfokus über `initialFocusRef`. `Dialog` nutzt den Hook, seine bestehenden Tests bleiben unverändert grün. |
...
| Bildfläche in `CurationLightbox` | Der Bildkasten hat genau die eingepasste Bildgröße. Die Bühne trägt `container-type: size`, der Kasten `aspect-ratio` aus `photo.aspect_ratio` und `width: min(100cqw, <ratio>·100cqh)` als numerischen Inline-Stil (wie in `PhotoGridTile`). Darin steht `PhotoImage variant="display"` in `size-full`. Fehlt `aspect_ratio`, füllt der Kasten die Bühne mit `object-contain`, und „neben das Bild“ ist dann nur noch der Backdrop. |
| `frontend/src/components/PhotoImage.tsx` | Neue optionale Prop `retryable`. Mit ihr zeigt der Fehlerzustand ein `Alert` mit „Erneut versuchen“, das einen internen Versuchszähler erhöht (Abhängigkeit des Lade-Effekts). Ohne die Prop bleibt alles wie bisher. Ladezustand (`Skeleton`) und Platzhalter bei 404 („wird noch verarbeitet“) gibt es schon. |
...
| `frontend/src/pages/AlbumDraftPage.tsx`, `frontend/src/pages/AlbumSelectionPage.tsx` | Rufen `useCurationLightbox()` auf und verdrahten die Kacheln. Am Seitenende steht `{photo && <CurationLightbox key={photo.id} … />}` mit `photo = items.find(…)`. In der Endauswahl kommt das Foto aus `items`, nicht aus der gefilterten Sicht. |
...
- **Foto verschwindet bei offener Großansicht** (z. B. Neuladen der Endauswahl): Erst wenn die Liste
  erfolgreich geladen ist und `openPhotoId` darin fehlt, ruft die Seite `close()` — es bleibt kein
...
| AK11 | Fokus-Rückgabe an das registrierte Element | Seiten: Fokus je Schließweg, lokaler Zustand, keine Neuladung | scrollY je Schließweg, Auslöser teilweise außerhalb des Sichtbereichs (`preventScroll`) |
