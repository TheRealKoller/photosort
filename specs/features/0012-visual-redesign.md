...
**Komponentenbibliothek:** Tailwind CSS + Radix UI + shadcn/ui, siehe ADR [`decisions/0011-ui-component-library.md`](../decisions/0011-ui-component-library.md). shadcn/ui-Komponenten werden als Quellcode nach `frontend/src/components/ui/` kopiert (kein zusätzliches Laufzeit-Paket "shadcn/ui"); tatsächliche neue `frontend/package.json`-Abhängigkeiten: `tailwindcss`, `@tailwindcss/vite`, `@radix-ui/react-*` (nur genutzte Primitives, schrittweise ergänzt), `class-variance-authority`, `clsx`, `tailwind-merge`. **Umsetzungskorrektur (PR "Tailwind-Fundament", 2026-08-03, siehe Nachtrag in ADR 0011):** kein `tailwind.config.ts`/`postcss.config.js`, kein `postcss`/`autoprefixer` — Tailwind v4 (aktuelle Major-Version) wird stattdessen über `@tailwindcss/vite` eingebunden, Design-Tokens leben direkt in `frontend/src/index.css` über `@theme` (CSS-first-Konfiguration, offizieller v4-Ansatz). Reine Tooling-Versions-/Mechanik-Entscheidung, keine Änderung der eigentlichen Bibliothekswahl.
...
**ADR:** [`decisions/0011-ui-component-library.md`](../decisions/0011-ui-component-library.md) (neu, Accepted).
...
**Umsetzung:** view-für-view in Gruppen (App-Shell/Login → Projekt-Seiten/FolderBrowser → Grid/Detail/Compare/RatingButtons), mind. 2 PRs, gemäß ADR 0011. Kein neuer Interaktionsablauf, keine neuen Bildschirme — reine visuelle/strukturelle Überarbeitung bestehender 9 Views plus Behebung der drei bestätigten funktionalen Lücken.
...
- **Tailwind CSS + Radix UI + shadcn/ui** statt Tailwind pur oder einer fertigen Komponentenbibliothek (Chakra, Mantine, MUI) — architect-Konsultation, 2026-08-02, siehe ADR 0011. Keine Rückfrage nötig, da technische Detailentscheidung innerhalb der vom Stakeholder gewünschten Richtung ("Bibliothek einführen, Wahl liegt bei dir").
