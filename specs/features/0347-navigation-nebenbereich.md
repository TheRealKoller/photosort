...
Der Gruppen-Trenner hängt **am Block der Hauptziele, nicht am Block der Nebeneinträge**: der Hauptziel-Block trägt `mb-2 border-b border-separator pb-2 lg:hidden`, damit verschwindet der Trenner ab `lg:` automatisch mit dem Block, den er abtrennt. Ein Trenner am Nebenblock bräuchte eine zweite, gegenläufige `lg:`-Regel zum Wieder-Abschalten — eine stille Fehlerquelle. `--separator` ist der dokumentierte Token für freistehende Linien auf dem Grund.
