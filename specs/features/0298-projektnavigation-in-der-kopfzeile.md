...
**E2E (`e2e/tests/project-nav.spec.ts`, neu).** Ausschließlich das, was jsdom prinzipiell nicht kann: der Breakpoint-Wechsel an der **exakten** Grenze (1024 px → genau vier sichtbare Ziele, kein Auslöser; 1023 px → genau umgekehrt; beide Messungen müssen sich unterscheiden) für AK5/AK6; die Kopfzeilenhöhe bei 360 px gegen dieselbe Kopfzeile ohne Projektbezug für AK7; Sichtbereichs-Enthaltung und Überlagerung des geöffneten Panels für AK12. Eigene Viewport-Breiten, an ein Playwright-Projekt gebunden (wie `grid-columns`). Rot-Nachweis bei Einführung im PR belegen. Erweitert statt neu: `tap-targets` nimmt den Menü-Auslöser und eine Panel-Zeile auf (AK11c).
