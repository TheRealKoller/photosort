...
**Status:** Implemented ([PR #345](https://github.com/TheRealKoller/photosort/pull/345))
**Erstellt:** 2026-09-06
...
- [x] **AK 7 — Der Guard-Test blockiert, statt nur zu warnen:** Der CI-Job `demo-scripts` ist Required Status Check der Branch Protection auf `main`. **Erfüllt am 2026-09-06:** `required_status_checks.contexts` lautet jetzt `["backend", "frontend", "docker-compose-check", "e2e", "demo-scripts"]`. Der ursprünglich hier vermerkte Fallstrick (`PUT` auf `/protection` ersetzt das gesamte Objekt und löscht bei unvollständigem Aufruf `enforce_admins`/`required_conversation_resolution`) wurde **konstruktiv umgangen** statt nur sorgfältig beachtet: Der Aufruf ging über den auf Status-Checks beschränkten Endpunkt `PATCH /repos/{owner}/{repo}/branches/{branch}/protection/required_status_checks`, der die übrigen Felder gar nicht erst berührt. Gesamtzustand vor und nach dem Aufruf gesichert und verglichen — außerhalb der `contexts`-Liste unverändert.
