...
**Status:** Implemented ([PR #56](https://github.com/TheRealKoller/photosort/pull/56))
**Erstellt:** 2026-08-08
...
- [x] AK1: In `.claude/agents/test-engineer.md`, Aufgabe 2/1, ist der Prüfpunkt "Wurde TDD tatsächlich befolgt …" (aktuell Zeile 37) vollständig entfernt, ohne Ersatz-Prüfpunkt mit gleichem Ritual-Fokus (Test-vor-Code-Reihenfolge). Die Phrase "tatsächlich befolgt" kommt in der Datei nicht mehr vor.
- [x] AK2: Das im gestrichenen Punkt enthaltene beobachtbare Symptom ("Tests wirken nachträglich an bestehenden Code angepasst") geht ausschließlich im bestehenden Testqualitäts-Punkt auf (Ergänzung von "keine übermockten Tests, die nur die Implementierung spiegeln" um "oder erkennbar nachträglich an die Implementierung statt an das beschriebene Verhalten angepasst wirken").
- [x] AK3: Aufgabe 2/1 enthält danach genau die fünf verbleibenden Prüfpunkte (AK-Abdeckung, Edge Cases, Testqualität inkl. AK2-Ergänzung, Coverage-Gate, Testkonzept-Abgleich) — keiner entfällt inhaltlich, kein neuer sechster Punkt entsteht.
- [x] AK4: Die Gesamtdatei ist gegenüber dem Ist-Stand (69 Zeilen) messbar kürzer — reines Streichen der einen Zeile aus AK1 allein genügt nicht; die Prosa-Abschnitte ("Warum diese Rolle", Aufgabe 3) sind erkennbar gestrafft. Aufgabe 1 und Aufgabe 3 behalten dabei jeden inhaltlichen Punkt der aktuellen Fassung mit erkennbarer Entsprechung in der neuen Fassung (nur Prosa/Redundanz gekürzt, kein Inhalt still gestrichen). Insbesondere bleiben erhalten: Coverage-Gate-Erwähnung (≥80%), Verweis auf `specs/architecture/0002-testkonzept.md`, Abgrenzung zu `security-engineer` (inkl. des Hinweissatzes "Fällt dir dennoch etwas Sicherheitsrelevantes auf, erwähne es kurz"), und die AskUserQuestion-Eskalationsregel bei Produkt-/Risiko-Trade-offs.
- [x] AK5: Die Modellzuordnung für `test-engineer`-Reviews bleibt "Standard" — weder ADR 0014 noch die Modelltabelle in `.claude/agents/developer.md` Schritt 4 noch das YAML-Frontmatter von `test-engineer.md` werden verändert. Explizit kein Bestandteil dieser Spec.
- [x] AK6: `.claude/agents/developer.md` Zeile 76 (Beschreibung des `test-engineer`-Aufrufs in Schritt 4) ist synchron angepasst: "TDD eingehalten" entfällt aus der Aufzählung, die übrigen dort genannten Prüfgegenstände (Abdeckung der Akzeptanzkriterien, Testqualität, Testkonzept-Abgleich, klassische Bugs/Logikfehler, Abweichungen von Code-Konventionen) bleiben unverändert stehen.
- [x] AK7: Verifiziert per (a) statischem Konsistenz-Check (AK1/AK2/AK3/AK5/AK6 gegen den finalen Diff abgeglichen) und (b) mindestens einem synthetischen Dry-Run-Review-Szenario auf einem Wegwerf-Branch mit einem konstruierten Diff, der bewusst "nachträglich wirkende" Tests enthält (schwache Assertions, die nur die Implementierung spiegeln) — dabei bestätigt sich, dass kein TDD-Ritual-Finding mehr erscheint, stattdessen das erweiterte Testqualitäts-Finding greift, und alle fünf verbleibenden Punkte im Review-Output erkennbar adressiert sind. Details siehe "Entscheidungen".
...
- **AK7(b)-Dry-Run-Ergebnis (developer-Umsetzung, 2026-08-09):** auf einem inzwischen gelöschten Wegwerf-Branch (`wegwerf/0025-test-engineer-dry-run`, von diesem Zwischenstand mit bereits aktualisiertem `test-engineer.md` abgezweigt) wurde ein synthetischer Diff mit einer bewusst tautologischen Test-Assertion angelegt (`apply_discount`-Beispiel, Assertion spiegelt die Implementierungsformel statt eines konkreten Erwartungswerts). Der real aufgerufene `test-engineer`-Agent meldete daraufhin **kein** TDD-Ritual-Finding mehr; stattdessen griff das erweiterte Testqualitäts-Kriterium ("erkennbar nachträglich an die Implementierung statt an das beschriebene Verhalten angepasst") als Hauptfinding, und alle fünf verbleibenden Prüfpunkte aus Aufgabe 2/1 wurden im Review-Output erkennbar adressiert (AK-Abdeckung, Edge Cases, Testqualität, Coverage-Gate, Testkonzept-Abgleich). Branch wie in der Teststrategie vorgesehen nach der Verifikation gelöscht, daher kein dauerhafter Code-Artefakt — dieser Absatz ist die bewusste Dokumentation des Ergebnisses, nachdem das Review (`requirements-engineer`, Schritt 4) die fehlende Nachvollziehbarkeit von AK7(b) im reinen Diff zu Recht bemängelt hatte.
