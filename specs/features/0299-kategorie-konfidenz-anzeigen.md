...
**Kennzeichnung als Selbsteinschätzung (Akzeptanzkriterium 7).** An beiden Anzeigeorten steht derselbe feste, im Code als benannte Konstante geführte Hinweis (`utils/confidenceLabels.ts`), erreichbar über einen Info-Auslöser. Der Auslöser trägt **sichtbaren Text** („Modell-Selbsteinschätzung") statt eines „i" mit `aria-label`: so sind sichtbarer und zugänglicher Name derselbe String (WCAG 2.5.3), und die Beschriftung sagt bereits im geschlossenen Zustand das Wesentliche. In den Bewertungsdetails ist der Auslöser ein natives `<details>/<summary>`, auf der Statistikseite das dort etablierte `InfoPopover` — der Aufklapp-Mechanismus unterscheidet sich, der Wortlaut nicht. Grund für das native Element: `CriterionDetailsList` wird ihrerseits innerhalb eines Popovers gerendert, dessen Hover-Grace-Bereich ein zweites portaliertes Popover mittragen müsste.

Der Hinweis erscheint **nur, wenn tatsächlich mindestens eine Zahl dargestellt wird** — eine Erklärung zu einer nicht vorhandenen Zahl wäre angesichts des Altbestands ohne Angabe auf absehbare Zeit der Regelfall und damit Rauschen. Wortlaut:
...
**Kuratierung.** Ein Filterschalter „Nur unsichere Zuordnungen" in der bestehenden Filterleiste, standardmäßig aus, über die vorhandene `Checkbox`-Komponente (Sicht-Entscheidung dieses Besuchs, kein Switch — der steht im Produkt für eine dauerhafte Einstellung). Ausschalten stellt exakt den vorherigen Sichtstand wieder her. Der Kategorie-Chip der Gruppenüberschrift bekommt keine Zahl.

Die Leertexte des aktiven Filters unterscheiden sich vom Erschöpfungshinweis („Kein weiteres Foto verfügbar") und decken **alle drei** Gruppierungsebenen ab, weil bei aktivem Filter auch Tag und Tageszeit leer laufen können und sonst fälschlich als erschöpft erschienen: Tag, Tageszeit und Partition. Eine Partition mit *weniger als* `topN` Fotos bekommt zusätzlich einen eigenen Text („Kein weiteres Foto unter 60 % in dieser Gruppe.") — dort fehlen die Plätze dem Filter, nicht dem Bearbeitungsstand. Alle Texte werden aus `LOW_CONFIDENCE_THRESHOLD` gebildet, damit Schwelle und Wortlaut nicht auseinanderlaufen.

**Statistikseite.** Neuer Abschnitt „Konfidenz der Kategorie-Erkennung" nach dem bestehenden Kategorienverteilungs-Block, in der Anzeigereihenfolge der Registry (nicht alphabetisch — konsistent zum vorhandenen Kategorienblock). Dargestellt als **Tabelle** (Kategorie | Fotos mit Angabe | Ø Sicherheit), gleiche Bauform wie der unmittelbar darüberstehende Kategorienverteilungs-Block; die Bezugsbasis steht als eigenes Kennzahlenpaar darunter, wie vom Feldsatz `photos_with_confidence`/`photos_without_confidence` vorgegeben. Beide Zähler beziehen sich auf **klassifizierte** Fotos (Klassifizierungszeilen), nicht auf alle Fotos des Projekts — Fotos ganz ohne Lauf stehen im Nachbarblock als „Nicht klassifiziert"; die Beschriftungen sagen das aus. Ein fehlender Mittelwert erscheint als Strich, nicht als leere Zelle und nicht als `0 %`. Der Mittelwert ohne ausgewiesene Basis wäre eine Zahl ohne Aussage — die Basis ist deshalb Pflichtbestandteil, nicht Beiwerk.
...
- **Technische Detailentscheidungen der Umsetzung** (in der Review-Phase geprüft und in die Abschnitte oben übernommen): explizites `"confidence": null` gilt als gelieferter, unbrauchbarer Wert und wird protokolliert, ein *fehlender* `confidence`-Schlüssel ist der stille Regelfall (Sentinel statt `None`); bei einem verworfenen Objekt-Eintrag geht nur der `key`-Anteil ins Log, nie der ganze Eintrag; natives `<details>/<summary>` in den Bewertungsdetails statt eines zweiten portalierten Popovers; der Kennzeichnungs-Hinweis erscheint nur bei tatsächlich dargestellter Zahl; Filter-Leertexte auf drei Gruppierungsebenen; Statistikblock als Tabelle mit ausgewiesener Basis.
