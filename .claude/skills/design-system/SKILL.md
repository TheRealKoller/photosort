...
- **Sticky Stepper-Fortschrittsnavigation** (noch nicht implementiert, vorgesehen für Spec 0042 — Pipeline mit eigener Route pro Schritt statt Fünf-Sections-auf-einer-Seite): horizontale `<nav><ol>`-Leiste, 5 Schritt-Kreise à 44×44px + Verbindungslinie, `sticky top-0` mit `bg-bg/95 backdrop-blur-sm border-b border-border`. Vier Zustände, Farbe nie alleiniges Signal: **erledigt** = gefüllt `--status-success` + Häkchen `--chip-fg`; **aktuell** = zusätzlicher `--accent`-Ring + fettes Label + `aria-current="step"` (bleibt bei Rückkehr zu einem erledigten Schritt zusätzlich grün gefüllt, kein Statusverlust durchs Betrachten); **ausstehend** = umrandeter neutraler Kreis, echter `<Link>`; **blockiert** = gedämpft (`opacity-50`) + Schloss-Icon, kein `<Link>` (`aria-disabled` + `tabIndex={-1}`), Grund über den Info-Popover-Trigger (siehe oben) statt Dauertext. Jedes Element mit vollem `aria-label` ("Schritt 2 von 5: …, erledigt"). Bleibt auf allen Breakpoints horizontal, Labels erst ab `sm:` sichtbar, darunter ersetzt durch eine Orientierungszeile über der Leiste. Erster Skip-Link im Produkt davor, damit Tastaturnutzung nicht durch alle 5 Einträge tabben muss.
