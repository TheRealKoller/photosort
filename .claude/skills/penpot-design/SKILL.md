...
**⚠ ZUERST AUF DIE SEITE DER BAUSTEINE WECHSELN — sonst schlägt der Lauf beim ersten Schreibzugriff fehl.** Penpot lässt nur die **aktive** Seite beschreiben: Ein Aufruf gegen eine Form auf einer anderen Seite endet mit `Value not valid: Cannot modify a page that is not currently active. Code: :fills` (2026-09-11 gemessen). Vorher also ermitteln, auf welcher Seite die Varianten-Behälter mit `schluessel` liegen, und dorthin wechseln; `penpot.openPage` und die Prüfung, ob der Wechsel gegriffen hat, gehören in **getrennte** Aufrufe.

Der Lauf selbst sucht seine Behälter **nur auf der aktiven Seite** (`findShapes` mit `penpot.root` als Wurzel) — und das ist die Vorbedingung dafür, dass der Satz „je Seite einmal anstoßen" überhaupt trägt: Ohne die Wurzel suchte er über alle Seiten, fände Behälter auch dort, wo er sie nicht ändern darf, und bräche beim ersten solchen Schreibzugriff mitten im Bestand ab. Was auf einer anderen Seite liegt, erscheint im Bericht als eigener Ausgang `aufAndererSeite`; liegen die Bausteine auf mehreren Seiten, wird Schritt K je Seite einmal angestoßen, und der Bericht nennt jeweils die Seite, über die er berichtet.

**Der Seitengrund gehört zur selben Nachführung.** Er steht in keiner Datei und wird von keinem Skript gesetzt. Die Seite der Bausteine trägt ihn als `page.background`, und er ist über einen Ad-hoc-Aufruf setzbar — **aber nicht token-gebunden:** Eine Seite ist keine Form, `applyToken` greift dort nicht. Der Wert wird deshalb aus dem Token `color.bg` **gelesen** und gesetzt, nie getippt. Das ist die eine Stelle des Entwurfsbildes, an der ein Farbwert ohne Bindung steht; sie ist als Lücke zu führen. Ohne diesen Schritt beurteilt die Sichtprüfung einen anderen Untergrund als den, gegen den der Kontrast gerechnet ist — beim ersten echten Lauf stand dort `#E8E9EA`, ein helles Grau, auf dem weiße Bretter gar nicht als Fehler auffielen.
