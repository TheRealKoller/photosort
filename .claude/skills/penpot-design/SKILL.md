...
**⚠ ZUERST AUF DIE SEITE DER BAUSTEINE WECHSELN — sonst schlägt der Lauf beim ersten Schreibzugriff fehl.** Penpot lässt nur die **aktive** Seite beschreiben: Ein Aufruf gegen eine Form auf einer anderen Seite endet mit `Value not valid: Cannot modify a page that is not currently active. Code: :fills` (2026-09-11 gemessen). Der Lauf sucht seine Behälter dagegen über **alle** Seiten und findet sie auch dort, wo er sie nicht ändern darf — der Fehlschlag kommt deshalb erst beim Schreiben, nicht beim Suchen. Vorher also ermitteln, auf welcher Seite die Varianten-Behälter mit `schluessel` liegen, und dorthin wechseln; `penpot.openPage` und die Prüfung, ob der Wechsel gegriffen hat, gehören in **getrennte** Aufrufe. Liegen die Bausteine je auf mehreren Seiten, läuft Schritt K je Seite einmal.
